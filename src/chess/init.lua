require("chess.input")
require("chess.checkmate")
require("chess.players")

---@class Chess
local chess = {
---@class piece_moves : Piece_moves
  piece_moves = require("chess.piece_moves"),
  ---@class board : Board
...
---@return boolean
function chess:SelectHuman()
  local x,y = self.matrice:GetPos(Input_getX()+(8*1.5),Input_getY()+2)
  x =x + 1
  y =y + 1 
  if self.players:GetCurantType()=="human" then
      if self.selector:IfSelect()==false then
          print("init select")
          if(Input_click() and self.matrice:GetId(x,y)>0 ) then
              ...
              self.piece_moves:InitMoves(self.matrice,x,y)
              end
          end
      end
  end
  ...
  if self.players:GetCurantType()=="human" then
      if self.selector:IfSelect()==true and Input_click() then      
          local sx,sy,_,_ = self.selector:Get()
          if not(sx==x and sy==y) then
              if self.piece_moves:IsMove(x,y) then
...
function chess:_drawCursor()
  ...
  if self.selector:IfSelect() then
    if Input_click2() then
      self.selector:Deselect()
      --self.listmoves = {}
      self.piece_moves:Reset()
    end
  end

  -- draw select
  self.selector:DrawMove(self.piece_moves:GetMoves())
...
--players
  if self.players:Update(self)==1 then
    self.checkmate:Update(self.matrice,self.players)
    self.matrice:Promote()
    self.piece_moves:Reset()
  end
